...
function kreativ_styles() {
	...
	$notice_bar_bg = get_theme_mod( 'notice_bar_bg_color', $default_notice );
	$notice_bar_font_color = get_theme_mod( 'notice_bar_font_color', $default_notice );
	...
	$notice_bar_font = '';
	$notice_bar_font .= ( $default_notice !== $notice_bar_font_color ) ? sprintf('
	...
	', $notice_bar_font_color ) : '';
	
	//Enqueue inline styles.
  if ( $notice_bar_font ) {
    wp_add_inline_style( 'custom-css', $notice_bar_font );
  }
	
	$notice_css = '';
	$notice_css .= ( $default_notice !== $notice_bar_bg ) ? sprintf( '
	  .notice-bar {
	    background: %1$s;
	  }
  ...
  // Enqueue inline styles.
  if ( $notice_css ) {
    wp_add_inline_style( 'custom-css', $notice_css );
  }
